...
sub new  {
    ...
    my $settings_file = exists $args{settings_file} ? $args{settings_file} : '/etc/vault' ;
    ...
    $self-> {'schema'} = VaultDB::Schema->connect('DBI:mysql:vault:'.$self-> {'host'}.':'.$self-> {'port'},$self-> {'settings'}->{database},$self-> {'settings'}->{password});
...
sub get_default_value {
  ...
  croak "Config option ($request) not set in config file (".$self-> {'config_file'}.')';
}

sub send_validation_email {
  my($self,$email,$email_validation,$username) = @_ ;

  my $tt = Template->new( EVAL_PERL => 1, INCLUDE_PATH => $self->get_default_value('email_template_dir'));
  my $data ="" ;
  my $vars   = {
  ...
  my $msg = MIME::Lite->new
    (
     Subject => $self->get_default_value('register_subject','Vault registration'),
